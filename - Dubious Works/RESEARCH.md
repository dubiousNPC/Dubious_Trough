...
A later pass added the HUD widget pair — MoonHUD and BSCompass, two on-screen
elements driven by a texture atlas, one tracking lunar phase and one tracking
camera heading. These are the source for §1.27-§1.32, §2.20-§2.23 and
§4.4-§4.7. They contribute the one category the suite had not covered: a
handler that is *legitimately* per-frame because its input is continuous and
un-eventable, where the saving is quantisation rather than an event. Their
`pcall` material is unusual too — five of the eight found were guarding a
condition the engine does not produce at all, which is checkable against the
engine's own log strings rather than by argument.
...
## 1.27 A HUD widget is an honest per-frame case — quantise the output, not the input

There is no event for "the player turned". A compass that tracks the camera has
to read it per frame, so by §1.11's test it qualifies. But that is only half the
answer, and the half that matters is the other one.

The widget does not display a heading. It displays **one of 36 frames**. So the
early-out is on the derived discrete state, never on the continuous input:
...
With 36 frames that fires once per 10° of turn. Standing still it never fires at
all. A full 360° spin costs 36 texture swaps, not one per frame at whatever the
framerate happens to be.

This is a third rung the ladder in §1.3 does not name, because it is not about
finding an event:

> **If the input is continuous and un-eventable, quantise it and early-out on
> the quantised value.** Decide the smallest number of distinct outputs that
> still looks right, and make that number — not the framerate — the cost.

The corollary is that frame count is a performance decision disguised as an art
decision. Going from 36 frames to 360 for smoothness multiplies the update rate
by ten. That was worth it here; it should be a decision, not a side effect.

Two details that make the rest of the handler honest:

- **Read the cheap thing.** `camera.getYaw()` is a scalar. The obvious
  alternative, `camera.viewportToWorldVector(v2(0.5, 0.5))`, does a matrix
  transform and degenerates when you look straight up or down. Both give a
  heading; only one is a per-frame cost worth paying.
- **Expose the sample rate.** A `SAMPLE_EVERY` setting gating how often the
  heading is even read costs three lines and lets someone on weak hardware buy
  the frames back. Default 2; entirely imperceptible while turning.

## 1.28 A resource constructor in `onFrame` is not a cost, it is a leak

`ui.texture` registers a resource on every call. Calling it per frame to cut a
tile out of an atlas therefore does not merely cost time — it accumulates.

Build the whole set once, at load, into a flat array:
...
360 textures at load, none afterwards. `onFrame` then only ever assigns
`props.resource = tiles[n]`.

The trap that comes with the array is the **cache key**. Where textures are cut
lazily and memoised, the key has to include everything that can change:

```lua
local id = path .. '|' .. cell .. '|' .. row .. '|' .. phaseIndex
```

Omit `cell` and a user who changes the tile size keeps being served cuts made at
the old one, with no error and nothing in the log.

## 1.29 Read settings from mirrored globals, not from storage

A `storage.playerSection(...):get(key)` per frame is far more expensive than a
global read, and a HUD handler may want six settings. Mirror them once on change
and read the mirror:
...
The mirror is not free of its own hazard. Changes fall into three classes and
conflating them is a real bug:

| Class | Example | Response |
|---|---|---|
| **retile** | atlas path, frame count, cell size | rebuild the texture array, then the tree |
| **rebuild** | border, padding, layout, panel shape | rebuild the element tree |
| **poke** | tint, opacity, position | assign `props` on the live element |

Miss a key from the retile set and changing the atlas leaves the widget drawing
frames cut from the previous sheet — the settings page says one thing and the
screen shows another.

## 1.30 Pre-create hidden elements; toggle, do not rebuild

Where a widget has optional layers that come and go — a lit cardinal glyph, an
overlay another mod raises — the instinct is to add and remove elements. That
rebuilds the tree during play.

Build all of them once, hidden, and toggle:

```lua
for key, path in pairs(geo.cardinals) do
    cardinalElements[key] = fullLayer('cardinal' .. key, path)   -- visible = false
end
```

Ten elements built at load. Raising one is `props.visible = true` plus a single
`:update()` for every change that frame. The element count is bounded by the
artwork, not by what happens in play.

The same reasoning applies to the `:update()` call itself: accumulate a `dirty`
flag across the handler and call it once, rather than once per thing changed.

## 1.31 Quantise a continuous ramp, or it becomes a per-frame update

An opacity ramp — a glyph fading in as you turn toward it — is continuous, so
recomputing it per frame means updating the element per frame. That is §1.27's
problem again, one layer down.

Quantise the ramp itself:

```lua
local step = math.floor(t * ALPHA_STEPS + 0.5)    -- ALPHA_STEPS = 16
```

and include the step in the change comparison. Measured across a full 360° turn
with four cardinals, two bands each: **129 distinct states, not 360**. Sixteen
steps is indistinguishable from smooth at HUD size.

The rule generalises: any continuous value driving a visual should be reduced to
the smallest set of distinct outputs that looks right, *before* it reaches the
change test. Otherwise the change test never fires false and you have written a
per-frame update with extra steps.

## 1.32 A cache keyed on a coarse bucket outlives the condition that filled it

This one is the bridge to Part 2, because removing a `pcall` is what exposed it.

A tracker cached its reading per quarter-hour of game time — sensible, since the
underlying value changes every few days. But `self.cell` is nil while a save
loads, so a reading taken during the load came from a **fallback tier**, and was
then cached under that bucket. The authoritative source became available
milliseconds later and was not consulted again for up to fifteen in-game
minutes.

```lua
-- Only cache a reading taken with a live cell.
if r and self.cell ~= nil then
    lastResult, lastResultDay, lastResultHour = r, day, hour
end
```

The general form: **do not cache a degraded result under a key that outlives the
degradation.** Either cache only results from the authoritative source, or make
the cache key include which tier produced it. A cache that cannot distinguish
"the answer" from "the best I could do at the time" will serve the second one
long after the first became available.
...
16. If it stays: does it print the error, and enough context to identify the
    subscriber or path? A `pcall` that discards the error is concealment.
17. Have you checked the engine's **log strings** for this failure? If the
    engine logs and continues, the guard is inert (§2.20).
18. Does the failure path write a sticky flag? Then one caught error lasts the
    session, not the call (§2.21).
19. Is it wrapped around a *parser*? Validate the input with a pattern instead;
    a `pcall` there cannot tell bad input from a broken parser (§2.22).
20. Is it in a file whose failure would remove the settings page? That is a real
    asymmetry, and the answer is still validation, not catching (§2.23).

## 2.20 Verify the call actually raises — the engine's own log is the evidence

Five `pcall`s in this pass wrapped `ui.texture`, guarding a missing texture file.
`ui.texture` does not raise on a missing file. OpenMW logs it:

```
Failed to open image: Resource 'textures/gherb/tx_comberry_01_p.dds' not found
```

and carries on. The binding raises on one thing only: a malformed argument — a
non-string `path`, or none at all — which is a bug in the calling script.

So the guard caught nothing it was written for, and everything it was not. A
typo in the argument shape became a silently blank widget instead of a stack
trace, and any future change to the binding would have been swallowed the same
way. That is §2.15's inert guard, but it comes with a **method**, which §2.15
did not:

> Before wrapping an engine call, search for the failure's log string. If the
> engine's response to your failure case is a log line, your `pcall` is inert
> and the fallback behind it is dead code.

Engine log strings are searchable — `Failed to open image`, `Failed to load`,
`Resource ... not found` are all in forum posts and issue trackers. That takes
five minutes and settles the question that otherwise gets settled by assumption.

The replacement is to check the one thing that can genuinely be wrong, and let
...
## 2.21 A failure path that latches turns one transient error into a session-long regression

The worst `pcall` in either mod:

```lua
local ok, moons = pcall(core.weather.getCurrentMoons, self.cell)
if not ok then
    engineAvailable = false     -- and never checked again
    return nil
end
```

`engineAvailable` gates the authoritative tier. One error — from any cause, at
any time — permanently demoted the mod to its fallback for the rest of the
session. No message. The fallback is good, so nothing looks wrong; it is simply
less accurate from that moment until the game is restarted.

This is §2.8's silent degradation with an aggravating factor worth naming
separately, because it changes the blast radius:

> **If the `pcall`'s failure path writes a sticky flag, the scope of one caught
> error is the session, not the call.**

A latch also destroys the evidence: by the time anyone notices the mod is less
accurate, the error that caused it is thousands of frames in the past.

The fix was to identify the one call shape that could plausibly raise — a nil
`self.cell`, which happens while a save loads — and check it directly:
...
A genuine engine fault now surfaces instead of being mistaken for "the binding
is broken, stop trying". And see §1.32: removing this `pcall` immediately
exposed a caching bug it had been masking, which is §2.10 with a fresh instance.

## 2.22 For parseable input, validate with a pattern

Two `pcall`s wrapped `util.color.hex`, which *does* raise, on input that is
*genuinely* untrusted — whatever the user typed into a settings text field. By
§2.3 that looks defensible.

It still is not, for a reason worth separating from the others: a `pcall` around
a parser catches malformed input **and** a fault in the parser, and cannot tell
them apart. Validate the input instead and leave the call bare:
...
Three things improved that the `pcall` version could not:

- The accepted grammar is now stated, so it can be tested. 24 assertions,
  including eight malformed inputs.
- Three-digit shorthand became trivial to add. Under `pcall` it would have been
  a second catch.
- A fault inside `util.color.hex` now raises, as it should.

The generalisation: **`pcall` is not a substitute for knowing what your input is
allowed to be.** If you can write the pattern, write the pattern.

## 2.23 The load-time blast radius, and why it does not argue for `pcall`

An uncaught error while a script loads stops the whole file. For a mod whose
settings are registered from that file, that means the settings page **and** the
widget both disappear, with nothing on screen to say why. Two real instances in
one pass:

- A settings file called `sliderArg()`, a helper that existed in the sibling mod
  it was copied from but not in this one. A nil call, at load.
- A settings group named a renderer from a mod that was not enabled.
  `I.Settings.registerGroup` fails, and takes the script with it.

The tempting conclusion is that load-time code should be defensive. It is the
wrong conclusion, but the near-miss underneath it is real:

> A bad value in a settings *text field* can brick the settings page it was
> typed into. The user cannot change it back, because the page is gone.

That is the one asymmetry in this document that genuinely favours caution — and
the right form of caution is §2.22's validation, not a `pcall`. Validation keeps
the page alive and the value visible; a `pcall` keeps the page alive and hides
which setting was at fault.

Two structural fixes are better than either:

- **Bundle the dependency rather than naming it.** Shipping the renderer inside
  the mod, registered from its own `.omwscripts`, removes the failure mode
  instead of handling it. Duplicate `registerRenderer` across mods is logged and
  ignored; a duplicate settings *group* is fatal, so namespace group keys.
- **Execute the file in a harness** (§4.4). Both of the above were found in
  seconds that way, and neither is visible to a syntax check.
...
| `load_check.lua` | **Load-time errors.** Executes the scripts against stubs and runs `onInit`/`onFrame`. Catches nil globals and bad registration, which no syntax check will (§4.4). |
...
## 4.4 Syntax checking does not catch a call to a nil global — execute the file

`luacheck.py` and `luac -p` check that the file parses. They will happily pass:

```lua
argument = sliderArg(0, 120, 1, 'px'),   -- sliderArg is not defined in this file
```

Both mods in this pass shipped exactly that, and the symptom is the worst kind:
no settings page and no widget, because the load error stops the file before
either is registered (§2.23).

`dev/load_check.lua` stubs the OpenMW API, `require`s the scripts for real, runs
`onInit` and `onFrame`, and reports what got registered:

```
--- scripts/moonhud/MH_hud.lua
  LOAD ERROR: MH_settings.lua:259: attempt to call a nil value (global 'sliderArg')
```

Two things it should check beyond "did it load":

- **Every renderer a setting names is both present on disk and listed as a MENU
  script.** Shipping the file without the manifest entry is silent breakage, so
  the two are verified separately and the file-without-manifest case is a hard
  error.
- **Settings values can be forced**, so code paths behind a non-default preset
  actually execute: `PRESEED="ATLAS_PRESET=dial_iron,LAYOUT=Triangle"`. A
  harness that only ever runs the defaults tests one configuration out of
  however many ship.

## 4.5 Separate a harness fault from a mod fault before "fixing" working code

Building the harness above surfaced a run of failures that all looked like mod
bugs and were all missing stubs: vector arithmetic metamethods,
`content:insert`, `ui._getMenuTransparency`, `scripts.omw.mwui.constants`,
`storage:setLifeTime`, `I.MWUI`, `core.l10n`, `input.CONTROLLER_BUTTON`,
`ui.screenSize`.

This is the inverse of §4.1. A mock that accepts everything hides bugs; a mock
that is merely *incomplete* invents them. Both end with the mod being blamed.

The discipline is the same one §2.17 states for a `pcall`'d call being blamed
for a failure it did not cause: **establish which side is wrong before changing
either.** A missing stub fails on the stub's own line, so the evidence is
usually right there in the message.

## 4.6 Verify the measurement before trusting a surprising result

Three times in one pass, a check reported a failure that did not exist.

- **A frame-by-frame image diff reported all 36 frames differing**, by up to 226
  levels. The frames were identical; the diff was comparing RGB in *fully
  transparent* pixels, where the value is undefined. `magick compare -metric AE`
  returned 0.
- **A needle's rotation rate measured −0.959°/frame against a target of −1.**
  The needle is near-symmetric, so the principal-axis estimator was picking
  whichever end had marginally more pixels. Measuring on flat-colour output,
  free of texture noise, gave −1.002 ± 0.201.
- **An overlay looked misaligned on screen.** Measured against the native
  composite it was 0.2px out.

None of these was a bug in the thing being measured. Each cost as much time as a
real bug would have.

> When a check reports something surprising, test the check before changing the
> code. A surprising failure is evidence about *either* side.

The corollary is that eyeballing and measuring are both unreliable, in opposite
directions, and the fix is to use each to check the other. Rendering the atlas
and looking at it caught clipped labels, two variants that were visually
identical, and tick marks that chained frames together — none of which any
assertion would have caught. Measuring caught the three above, which looking
got wrong.

## 4.7 A check that has never failed has not been shown to work

The renderer bundle check in §4.4 passed the moment it was written, which proves
nothing. It was then negative-tested both ways: remove the `MENU:` line with the
file present, and delete the file with the line present. The first must be a
hard error, the second a warning. Both fired.

Any check whose failure path has never executed is an assumption with a test
around it.
...
19. Is the handler's input continuous and un-eventable? Then quantise it and
    early-out on the quantised value — the frame count, not the framerate, is
    the cost (§1.27, §1.31).
20. Are you constructing a resource in the handler? `ui.texture` registers one
    per call; build the set at load (§1.28). If you memoise, does the key
    include everything that can change?
21. Are you reading `storage` in the handler? Mirror to globals on change, and
    split the change classes into retile / rebuild / poke (§1.29).
22. Are you adding or removing UI elements during play? Build them hidden and
    toggle instead (§1.30).
23. Are you caching a reading that might have come from a fallback? Do not cache
    it, or key it by tier — a degraded answer outliving the degradation is
    invisible (§1.32).
24. Did you run `load_check.lua`, not just the syntax checker? A call to a nil
    global passes `luacheck.py` and removes the settings page (§4.4, §2.23).
25. Did a check report something surprising? Test the check first (§4.6). Has
    its failure path ever actually executed (§4.7)?
